/**
 * Zano remote-node pool + selection / health / hot-swap.
 *
 * Structural mirror of `zph-nodes.ts` — same 2-tier shape (user pin →
 * hand-curated default list, no runtime community-fallback tier). One
 * difference in the probe layer, not this file: `zano_probe_node`
 * (`src-tauri/src/zano_rpc.rs`) speaks Zano's JSON-RPC `getinfo`, not the
 * Monero-family REST `/get_info` the shared `wallet_rpc_common::probe_node`
 * assumes — verified live 2026-08-27 that the REST form 404s against a real
 * Zano daemon. This module only calls the Tauri command, so it's naturally
 * insulated from that difference; documented here so nobody "simplifies" it
 * back onto the shared Monero-shaped probe.
 *
 * We only contact these nodes when Zano is the active chain AND a wallet
 * session is being established — never on app startup.
 */

import { invoke } from "../lib/tauri";
import { getStore } from "../store";
import { ZANO_DEFAULT_NODES } from "./zano-nodes-default";

export interface ZanoNode {
  url: string;
  operator: string;
}

/** Re-export of the baked-in pool for the Settings UI. */
export const TRUSTED_ZANO_NODES: ReadonlyArray<ZanoNode> = ZANO_DEFAULT_NODES;

/** Store key holding the user-pinned daemon URL ("" = auto). */
const SELECTED_NODE_KEY = "zano_selected_node";
...
// =========================================================================
// Rust-backed probe (bypasses webview CORS; speaks Zano's JSON-RPC getinfo)
// =========================================================================
...
async function probeNodeRust(
  ...
  return invoke<RustProbeResult>("zano_probe_node", {
    url,
    timeoutMs,
  });
}
...
/**
 * Merged active pool. Just the baked-in single-node list — Zano has no
 * runtime community directory to fall back to (see
 * `zano-nodes-default.ts`'s header). Reserved as an async function so a
 * future runtime source can be added without touching every call site, the
 * same reservation `zph-nodes.ts` makes for the same reason.
 */
export async function getActivePool(): Promise<ZanoNode[]> {
  return [...TRUSTED_ZANO_NODES];
}
...
/**
 * Hot-swap threshold (same rationale as Monero/Zephyr's): only swap to a
 * new node if it's at least 1.75x faster than the current one, to avoid
 * flapping on ordinary latency variance.
 */
 ...
 * blocks of the max seen — prevents picking a stuck node whose latency
 * looks good only because it's serving stale data.
 */
 ...
 * Throws if everything fails, with a combined error message.
 */
export async function tryNodes<T>(
  ...
  throw new Error(`All Zano remote nodes failed:\n${errors.slice(0, 20).join("\n")}`);
}

/**
 * Race the pool in parallel via `probeNodeRust` and return the URL of the
 * first node to answer `getinfo` with OK status. Cached in `cachedNodeUrl`.
 * If the health cache has a fresh best node, return it without re-probing.
 */
export async function raceBestNode(timeoutMs: number = 5000): Promise<string> {
  const pinned = await getSelectedNode();
  ...
  const pool = await getActivePool();
  if (pool.length === 0) {
    throw new Error("Zano node pool is empty.");
  }
  ...
  try {
    ...
    throw new Error(`No Zano node reachable (tried ${pool.length} default node(s)).`);
  }
}

/**
 * Convenience for `initZanoSession`: the daemon URL to actually connect the
 * sidecar to — user pin, else the (currently single-entry) default pool via
 * `raceBestNode`. Named distinctly from `raceBestNode` so callers reaching
 * for "just give me a daemon URL" don't have to know the pin/race split.
 */
export async function pickZanoDaemon(timeoutMs: number = 5000): Promise<string> {
  return raceBestNode(timeoutMs);
}
